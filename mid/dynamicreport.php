...
if ($authTypeId == "1"){
    $sqlcode = mysql_query("CALL `marshalAuthsByPerson`($regionId, $authTypeId);");
}
if ($authTypeId == "3"){
    $sqlcode = mysql_query("CALL `armoredAuthsByPerson`($regionId, $authTypeId);");
}
if ($authTypeId == "5"){
    $sqlcode = mysql_query("CALL `rapierAuthsByPerson`($regionId, $authTypeId);");
}
if ($authTypeId == "6"){
    $sqlcode = mysql_query("CALL `youthAuthsByPerson`($regionId, $authTypeId);");
}
if ($authTypeId == "7"){
    $sqlcode = mysql_query("CALL `coursingAuthsByPerson`($regionId, $authTypeId);");
}
if ($authTypeId == "8"){
    $sqlcode = mysql_query("CALL `equestrianAuthsByPerson`($regionId, $authTypeId);");
}
